...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (light) {
      // Light mode: just instantiate client and return ok
      return NextResponse.json({ ok: true, mode: 'light' });
    }

    // Heavy mode: list domains (no emails sent)
    const domains = await resend.domains.list();
    return NextResponse.json({
      ok: true,
      domains: (domains as any)?.data?.length ?? null,
    });
  } catch (e: any) {
    return NextResponse.json(
      { ok: false, error: e.message || 'resend_check_failed' },
      { status: 500 }
    );
  }
}
